...
def split_user_input_and_find_word(user_input, word_to_find):
    # Split the input string into lines, removing leading/trailing whitespace
    categories = [line.strip() for line in user_input.split('\n') if line.strip()]
    ...
    for category_index, category in enumerate(categories, start=1):
        ...
        print(f"Debug: category_index={category_index}, category={category}, word_to_find={word_to_find}")
        print(f"Debug: type(category)={type(category)}, type(word_to_find)={type(word_to_find)}")
